...
public:
  ...
  static series_t
  generate_rand_series(uint32_t city_count);

  static series_t
  generate_natural_series(uint32_t n);
  ...
  template<typename M,
           typename S,
           typename =
             typename std::is_same<typename std::decay<S>::type, series_t>::type>
  static tsp64_t
  calculate_cost(S&& series, matrix<M>& m)
  {
