...
namespace pwrgl::egg
{
	...
	{
	    return  -PI*(90*pow(u,5) -255*pow(u,4) + 270*pow(u,3) - 180*pow(u,2) +45*u)*cos(PI * v);
	}
	...
	void generate_object()
	{
	   ...
	   //          l = vec_length(normal[u][v]);

	   //          double T = (normal[u][v][0] * LIGHT_SOURCE[0] +
	   //          		    normal[u][v][1] * LIGHT_SOURCE[1] +
	   //          		    normal[u][v][2] * LIGHT_SOURCE[2])/(LIGHT_SOURCE_VEC_LEN + l);
				
				// double T_atan = acos(T) * 180.0 / PI;

	            // printf("%f %f %f %f %f %f\n", l, T , T_atan, space[u][v][0], space[u][v][1], space[u][v][2] );
